...
int CursesInterface::showMenu(string str,vector<MenuItem> menulist,int def_select)
{
	...
	if (max_help_lines>0) {
		...
		wbkgd(progressWindow, COLOR_PAIR(10));
		box(progressWindow, ACS_VLINE, ACS_HLINE);
		wmove(progressWindow, 0,box_width-utf8strlen("Описание")/2);
		waddstr(progressWindow, "Описание");
	}
...
string ncGetOpenFile(string defaultDir) {
	...
	while(selection!=-1) {
		...
		if (dirList.size() != types.size()) ncInterface.showMsgBox("WTF!");
		items.clear();
		...
		selection = ncInterface.showMenu("[ " + currentDir + " ]", items);
